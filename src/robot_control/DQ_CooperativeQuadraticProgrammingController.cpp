...
DQ_CooperativeQuadraticProgrammingController::DQ_CooperativeQuadraticProgrammingController(const std::shared_ptr<DQ_CooperativeDualTaskSpace> &cdts,
                                                                     const std::shared_ptr<DQ_QuadraticProgrammingSolver> &solver):
    DQ_CooperativeKinematicConstrainedController(cdts),
    qp_solver_sptr_(solver)
{

}

VectorXd DQ_CooperativeQuadraticProgrammingController::compute_setpoint_control_signal(const VectorXd &q, const VectorXd &task_reference)
{
    return DQ_CooperativeQuadraticProgrammingController::compute_tracking_control_signal(q,task_reference,VectorXd::Zero(task_reference.size()));
}

VectorXd DQ_CooperativeQuadraticProgrammingController::compute_tracking_control_signal(const VectorXd &q, const VectorXd &task_reference, const VectorXd &feed_forward)
{
    if(is_set())
    {
